<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="flex justify-between items-center p-5 border-b border-gray-300">
          <h2 class="text-[1.375rem] font-bold m-0">{{ __ ('Doctors') }}</h2>
        ...
        <table class="w-full border-collapse">
          ...
          <tbody>
            ...
            <tr>
              <td class="p-4 text-left border-b border-gray-300 font-medium">{{ $doctor->name }}</td>
              <td class="p-4 text-left border-b border-gray-300 font-medium">{{ $doctor->last_name }}</td>
              <td class="p-4 text-left border-b border-gray-300 font-medium">{{ $doctor->middle_name }}</td>
              <td class="p-4 text-left border-b border-gray-300 font-medium">{{ $doctor->email }}</td>
              <td class="p-4 text-left border-b border-gray-300 font-medium">{{ $doctor->creator->name }} {{ $doctor->creator->last_name }}</td>
              <td class="p-4 text-left border-b border-gray-300 font-medium"><span class="status px-2.5 py-1.5 rounded-full text-white text-[0.9em] {{ $doctor->status }}">{{ __($doctor->status) }}</span></td>
              <td class="p-4 text-left border-b border-gray-300 font-medium">
                <div class="flex items-center gap-2.5">

                  <x-action-button class="bg-[#267EB2] text-white hover:bg-[#154E7A]" data-id="{{ $doctor->id }}" onclick="openEditModal(this)">
                  ...
                  @php
                  $deleteRoute = route('doctor.delete', ['id' => $doctor->id]);
                  @endphp
  ...
  <div id="createUserModal" class="hidden fixed top-0 left-0 w-full h-full justify-center items-center" style="background: rgba(0, 0, 0, 0.5);">
    <div class="bg-white p-5 rounded-lg w-[500px] max-w-[90%]">
      <h3 class="text-lg font-semibold mb-4">{{ __('Create doctor') }}</h3>
